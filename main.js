...
const generatePokemons = (data) => {
    ...
    for(let i = 0; i < data.length; i++){
        ...
        statHp.innerText = data[i].stats[0].stat.name + ': ' + data[i].stats[0].base_stat;
        statAttack.innerText = data[i].stats[1].stat.name + ': ' + data[i].stats[1].base_stat;
        statDefense.innerText = data[i].stats[2].stat.name + ': ' + data[i].stats[2].base_stat;
        statSpAttack.innerText = data[i].stats[3].stat.name + ': ' + data[i].stats[3].base_stat;
        statSpDefense.innerText = data[i].stats[4].stat.name + ': ' + data[i].stats[4].base_stat;
        statSpeed.innerText = data[i].stats[5].stat.name + ': ' + data[i].stats[5].base_stat;
        ...
        pokemonStats.append(pokemonAbilityName, pokemonAbilities , stats, pokemonImgBack, hideStats);
        pokemonWrapper.append(pokemonStats, pokemonName, pokemonImg, pokemonTypes, showStats);
        wrapper.append(pokemonWrapper);
        container.append(wrapper);
        stats.append(statHp, statAttack, statDefense, statSpAttack, statSpDefense, statSpeed);
